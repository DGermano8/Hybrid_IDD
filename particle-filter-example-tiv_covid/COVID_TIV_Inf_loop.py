...
# inst = list(pypfilt.load_instances("config/cli-tiv-ode.toml"))[0]

# patient_list = ['432192', '443108', '444332', '444391', '444633', '445602', '451152']
patient_list = ['432192', '443108', '444332', '444391', '444633', '445602', '451152']

for ii in range(0, len(patient_list)):
    inst = list(pypfilt.load_instances("config/cli-refractory-tiv-jsf.toml"))[0]
    patient_id = 'Good/' + patient_list[ii]

    out_dir = 'outputs/' + patient_id + '_3/' +  inst.settings['components']['model'] + '_' + str(inst.settings['filter']['particles'])
